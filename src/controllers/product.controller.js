...
export const getProductById = async (req, res, next) => {
  try {
    ...
    if (!response) res.status(404).json({ msg: `no se pudo encontrar el producto ${pid}` });
...
export const createProduct = async (req, res, next) => {
  try {
    ...
    if (!newProduct) res.status(404).json({ msg: "no se pudo crear el producto" });
...
export const updateProduct = async (req, res, next) => {
  try {
    ...
    if (!productUpdate) res.status(404).json({ msg: "no se pudo actualizar el producto" });
...
export const deleteProduct = async (req, res, next) => {
  try {
    ...
    if (!productToDelete) res.status(404).json({ msg: "no se pudo borrar el producto" });
    else res.status(200).json({ msg: `El Producto con id: ${pid} fue borrado con exito`});
  } catch (error) {
    next(error.message);
  }
};