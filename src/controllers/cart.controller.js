...
export const createCart = async (req, res, next) => {
    try {
      ...
      if (!newCart) res.status(404).json({ msg: "carro no creado" });
...
export const getAllCarts = async (req, res, next) => {
  ...
  export const updateCart = async (req, res, next) => {
    try {
      ...
      if (!cartUpd) res.status(404).json({ msg: "error al actualizar el carro " });
  ...
  export const deleteCart = async (req, res, next) => {
    try {
      ...
      if (!cartDel) res.status(404).json({ msg: "no se borro el carro " });
...
export const addProductToCart = async (req, res, next) => {
  ...
  export const removefromCart = async (req, res, next) => {
    try {
      ...
      if (!delProdToUserCart) res.json({ msg: "el producto NO se borro del carro" });
      else res.json({msg: `el producto ${pid} fue eliminado del carro`});
    } catch (error) {
      next(error.message);
    }
  };
  ...
  export const purchase = async (req, res) => {
    ...
    try {
        ...
        if (ticketResponse) {
            ...
            return res.status(400).send("No se genero el ticket");
        }
    } catch (error) {
        console.error(error);
